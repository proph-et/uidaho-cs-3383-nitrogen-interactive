...
public class Phase2 : BossPhase
{
    ...
    protected override void BuildGraph()
    {
        ...
        var SpawnEnemies = new BehaviorNode(
            name: "SpawnEnemies",
            type: AbilityType.Summon,
            baseProb: 0.4f,
            range: 999f,
            isRequired: true,
            condition: () => boss.Abilities.SpawnEnemies.CanExecute(boss),
            action: () => { },
            executeAsync: (onComplete) =>
            {
                boss.Abilities.SpawnEnemies.Execute(boss, onComplete);
            }
        );
        ...
        var SpawnEnemiesIn = new BehaviorPath("SpawnEnemiesIn", 0.7f);
        SpawnEnemiesIn.AddStep(SpawnEnemies);
        SpawnEnemiesIn.FinalizePath();

        // initialize graph
        graph.AddPath(aggressive);
        graph.AddPath(SpawnEnemiesIn);
