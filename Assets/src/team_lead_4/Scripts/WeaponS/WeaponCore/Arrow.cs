...
public class Arrow : MonoBehaviour
{
    ...
    private float
        lifetime = 10f; // Holds the lifetime of the arrow. Controls how long the arrow is in the world before being destroyed.
    ...
    void Update()
    {
        if (!collisionActive &&
            Time.time - spawnTime >=
            collisionDelay) // Check to see if the difference between when the arrow was spawned and the current time is more than the collision delay.
        {
    ...
    void FixedUpdate()
    {
        if (!hasHit && arrowRB.linearVelocity.sqrMagnitude >
            0.01f) // Keep moving if we haven't hit anything adn we haven't run out of velocity
        {
            transform.forward = Vector3.Lerp(transform.forward, arrowRB.linearVelocity.normalized,
